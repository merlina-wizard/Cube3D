...
int	check_surroundings(char **map, int h, int w, int i, int j)
{
...
int	check_top_bottom_borders(char **map, int height, int width)
{
	int	i;
...
int	validate_map_loop(char **map, int h, int *p_count, t_player *p)
{
	int	i;
	int	j;
	int	width;
	...
	while (i < h)
	{
		...
		while (map[i][j])
		{
			...
			if (ft_strchr("0NSEW", c) && !check_surroundings(map, h, width, i, j))
